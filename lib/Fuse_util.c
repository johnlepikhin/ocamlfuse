...
#define create_ARGS (const char *path, mode_t mode, struct fuse_file_info *fi)
#define create_RTYPE int
#define create_CB vpath = copy_string(path); vres=callback2(*create_closure,vpath,Val_int(mode));
#define create_RES if (Field(vres,0) != Val_int(0)) fi->fh = Int_val(Field(Field(vres,0),0));
